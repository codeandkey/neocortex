...
static int search_tree_size = NC_SEARCH_TREESIZE;

void* ncSearchRun(void*);
int ncSearchSelect(ncPosition* pos, ncNode* root, ncNode* tree, int* nodes);
...
void* ncSearchWorkerMain(void* w)
{
    ...
    self->tree = (ncNode*) malloc(sizeof(ncNode) * NC_SEARCH_TREESIZE);
    ...
    for (int i = 0;; ++i)
    {
        ...
        int depth = ncSearchSelect(&self->pos, self->tree, self->tree, &self->treelen);
...
void* ncSearchRun(void* unused)
{
    ...
    for (int i = 0; i < num_search_workers; ++i)
    {
        ...
        pthread_create(&search_workers[i].handle, NULL, ncSearchWorkerMain, &search_workers[i]);
    }

    while (1)
    {
        if (milliseconds() - ttime < NC_SEARCH_TEST_INTERVAL)
            continue;
...
int ncSearchSelect(ncPosition* pos, ncNode* root, ncNode* tree, int* nnodes)
{
    ...
    if (root->cache != UNKNOWN)
    {
        if (root->cache != FULL)
        {
            ncSearchBackprop(root, root->cache);
            return 0;
        }
        ...
        int retdepth = 1 + ncSearchSelect(pos, best_child, tree, nnodes);
    ...
    root->cache = FULL;
    return 1 + ncSearchSelect(pos, root, tree, nnodes);
}
